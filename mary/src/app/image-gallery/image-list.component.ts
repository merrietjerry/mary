...
import { TriggerLight, loaderStart, loaderStop } from '../../assets/js/custom';
...
export class ImageListComponent implements OnInit{
    ...
    getImageList() {
        this._drupalService.getGalleryImages(this.galleryPath).subscribe(res => { this.node = res[0]; this.processData(); loaderStop() })
    }

    processData() {
        if (this.node.field_cover_image) {
            this.title = this.node.title;
            this.titleService.setTitle(this.title +' | ' + this._drupalService.siteName);
